...
sudo $aptbin -y install gparted tree mc fonts-inconsolata ack-grep cups-pdf nautilus-open-terminal 
sudo $aptbin -y install gedit-plugins 
sudo $aptbin -y install gedit-developer-plugins
sudo $aptbin -y install samba samba-doc
...
read -p "Install unity-chromium-extension ? " yn
case $yn in
    [YyOo]* ) 
