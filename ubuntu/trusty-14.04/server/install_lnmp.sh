...
do
    case $opt in
        ...
        "MariaDB 5.5")
            ...
            sudo add-apt-repository -y 'deb http://mirrors.linsrv.net/mariadb/repo/5.5/ubuntu trusty main'
            break;;
        "MariaDB 10.0")
            sqldb='mariadb'
            sudo $aptbint -y install software-properties-common
            sudo apt-key adv --recv-keys --keyserver hkp://keyserver.ubuntu.com:80 0xcbcb082a1bb943db
            sudo add-apt-repository -y 'deb http://ftp.igh.cnrs.fr/pub/mariadb/repo/10.0/ubuntu trusty main'
            break;;
        *)
            sqldb='mysql'
            break;;
